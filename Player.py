class Player:
   ...
   def calc_Strength(self, deck):
      ...
      '''
            ...
            2 - Two Pair
            1 - Pair
            0 - High Card - Go by highest points
      '''
      ...
      print(f'Hand Ranking: {self.getStrength()}')
      print('-----------------------------------')
      return 0
   ...
   def getInfo(self,deck):
      ...
      print(f'Straight? {is_a_straight(board_hand_val)}')
      print()


def is_a_straight(values):
   # create a copy to find a straight
   int_vals = values
   ...
   '''
   # Use a set to remove duplicates and allow fast lookup
   unique_values = set(values)

   # We only need to check ranges based on the unique values
   for value in unique_values:
      # Check if this number is the start of a 5-number sequence
      count = 0
      for i in range(5):
         if value + i in unique_values:
            count += 1
         else:
            break
      if count == 5:
